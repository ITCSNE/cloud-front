...
export const NewsFeedProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addNewsFeed = async (newsFeed: NewsFeed) => {
    return await axios
      .post("https://cloud-back-nlgv.onrender.com/api/auth/newsfeeds", newsFeed, {
        headers: {
  ...
  const updateNewsFeed = async (updatedNewsFeed: NewsFeed) => {
    return await axios
      .put(
        `https://cloud-back-nlgv.onrender.com/api/auth/newsfeeds/${updatedNewsFeed._id}`,
        updatedNewsFeed,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const deleteNewsFeed = async (id: string) => {
    return await axios.delete(`https://cloud-back-nlgv.onrender.com/api/auth/newsfeeds/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
  ...
  const getNewsFeeds = async () => {
    const response = await axios.get(
      "https://cloud-back-nlgv.onrender.com/api/auth/newsfeeds"
    );
