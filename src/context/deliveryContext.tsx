...
export const DeliveryProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addDelivery = async (delivery: Delivery) => {
    return await axios
      .post("https://cloud-back-nlgv.onrender.com/api/auth/add/drivers", delivery, {
        headers: {
  ...
  const updateDelivery = async (updatedDelivery: Delivery) => {
    return await axios
      .put(
        `https://cloud-back-nlgv.onrender.com/api/auth/drivers/${updatedDelivery._id}`,
        updatedDelivery,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const getDeliveries = async () => {
    const response = await axios.get("https://cloud-back-nlgv.onrender.com/api/auth/drivers");
    setDeliveries(response.data);
  };

  const deleteDelivery = async (id: string | undefined) => {
    return await axios
      .delete(`https://cloud-back-nlgv.onrender.com/api/auth/drivers/${id}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
